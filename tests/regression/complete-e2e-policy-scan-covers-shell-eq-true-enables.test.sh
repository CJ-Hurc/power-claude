...
# CERTIFIED=true / ALLOW_UNPROVEN=true enables under scripts/*.sh (and peers).
...
# matched CERTIFIED=1 / ALLOW_UNPROVEN=1 / "certified = true" (spaced) while
# the no-space =true env form stayed invisible in required product scripts/.
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
ENFORCE_PY="$ROOT/scripts/enforce/run.py"
RELEASE_PY="$ROOT/scripts/release-ready/run.py"
[[ -f "$ENFORCE_PY" ]] || { echo "FAIL policy-scan-shell-eq-true: missing enforce/run.py" >&2; exit 1; }
[[ -f "$RELEASE_PY" ]] || { echo "FAIL policy-scan-shell-eq-true: missing release-ready/run.py" >&2; exit 1; }

# Source must build shell =true needles (concatenated — never contiguous banned literals).
grep -qF '+ "=true"' "$ENFORCE_PY" \
  || { echo "FAIL policy-scan-shell-eq-true: enforce missing =true needle concat" >&2; exit 1; }
grep -qF '+ "=true"' "$RELEASE_PY" \
  || { echo "FAIL policy-scan-shell-eq-true: release-ready missing =true needle concat" >&2; exit 1; }
grep -qF 'ALLOW_UNPROVEN' "$ENFORCE_PY" \
  || { echo "FAIL policy-scan-shell-eq-true: enforce missing ALLOW_UNPROVEN" >&2; exit 1; }
grep -qF 'certified' "$ENFORCE_PY" \
  || { echo "FAIL policy-scan-shell-eq-true: enforce missing certified=true construction" >&2; exit 1; }

tmp="$(mktemp -d "${TMPDIR:-/tmp}/pc-policy-scan-shell-eq-true-XXXXXX")"
trap 'rm -rf "$tmp"; rm -f "$ROOT/scripts/complete-e2e/.policy-scan-plant-certified-eq-true.sh"' EXIT

PLANT="$ROOT/scripts/complete-e2e/.policy-scan-plant-certified-eq-true.sh"
# Shell =true enable shape (not CERTIFIED=1 / not spaced "certified = true").
printf '%s\n' '#!/usr/bin/env bash' '# plant: policy-scan shell-eq-true regression' 'CERTIFIED=true' >"$PLANT"
...
# Fixed floors must construct shell =true needles (concat form).
assert '"=true"' in src or "+ \"=true\"" in src or "+ '=true'" in src or '+ "=true"' in src, (
    "enforce must build shell =true needles"
)
assert "ALLOW_UNPROVEN" in src and "certified" in src, "enforce must build ALLOW_UNPROVEN/=true and certified=true"


def scan(roots: list[str], suffixes: set[str], *, shell_eq_true: bool) -> list[str]:
    banned: list[str] = []
    for root_name in roots:
        ...
        for path in sorted(base.rglob("*")):
            ...
            elif allow_jm in text or allow_jnm in text:
                banned.append(rel + ": " + allow_a)
            elif shell_eq_true and (allow_t in text or allow_ts in text):
                banned.append(rel + ": " + allow_a)
            cert_a = "CERTIFIED" + "=1"
            ...
            if cert_a in text or cert_b in text.lower():
                ...
                banned.append(rel + ": fake CERTIFIED enable")
            elif shell_eq_true and cert_t in text.lower():
                banned.append(rel + ": fake CERTIFIED enable")
    return banned


# Theater-kill: pre-=true needles must miss the CERTIFIED=true plant.
miss = scan(["scripts", "devtools", "configs", "media"], suffix_set, shell_eq_true=False)
assert not any(str(plant.relative_to(root)) in b for b in miss), (
    "sanity: pre-=true needles unexpectedly caught CERTIFIED=true plant: " + repr(miss)
)

# Fixed needles must catch CERTIFIED=true in the planted scripts/*.sh.
hit = scan(["scripts", "devtools", "configs", "media"], suffix_set, shell_eq_true=True)
plant_hits = [b for b in hit if str(plant.relative_to(root)) in b]
assert plant_hits, (
    "policy scan with shell =true needles must catch CERTIFIED=true "
...
print("live scan clean; scripts shell CERTIFIED/ALLOW_UNPROVEN=true enables covered")
PY

echo "PASS complete-e2e-policy-scan-covers-shell-eq-true-enables: shell CERTIFIED/ALLOW_UNPROVEN=true enables fail-closed in enforce+release-ready policy scan (no scripts/*.sh =true-shape blind spot)"
