...
# CERTIFIED="1" / ALLOW_UNPROVEN="1" (quoted) enables under scripts/*.sh (and peers).
...
# matched CERTIFIED=1 (unquoted) / CERTIFIED = 1 / =true while quotes broke the
# contiguous =1 needle — quoted numeric env form stayed invisible in required
# product scripts/.
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
ENFORCE_PY="$ROOT/scripts/enforce/run.py"
RELEASE_PY="$ROOT/scripts/release-ready/run.py"
[[ -f "$ENFORCE_PY" ]] || { echo "FAIL policy-scan-shell-quoted-eq-1: missing enforce/run.py" >&2; exit 1; }
[[ -f "$RELEASE_PY" ]] || { echo "FAIL policy-scan-shell-quoted-eq-1: missing release-ready/run.py" >&2; exit 1; }

# Source must build shell quoted ="1" / ='1' needles (concatenated — never contiguous banned literals).
grep -qF '="1"' "$ENFORCE_PY" \
  || { echo "FAIL policy-scan-shell-quoted-eq-1: enforce missing quoted =\"1\" needle concat" >&2; exit 1; }
grep -qF "='1'" "$ENFORCE_PY" \
  || { echo "FAIL policy-scan-shell-quoted-eq-1: enforce missing quoted ='1' needle concat" >&2; exit 1; }
grep -qF '="1"' "$RELEASE_PY" \
  || { echo "FAIL policy-scan-shell-quoted-eq-1: release-ready missing quoted =\"1\" needle concat" >&2; exit 1; }
grep -qF "='1'" "$RELEASE_PY" \
  || { echo "FAIL policy-scan-shell-quoted-eq-1: release-ready missing quoted ='1' needle concat" >&2; exit 1; }
grep -qF 'cert_dq' "$ENFORCE_PY" \
  || { echo "FAIL policy-scan-shell-quoted-eq-1: enforce missing cert_dq" >&2; exit 1; }
grep -qF 'cert_dq' "$RELEASE_PY" \
  || { echo "FAIL policy-scan-shell-quoted-eq-1: release-ready missing cert_dq" >&2; exit 1; }
grep -qF 'allow_dq' "$ENFORCE_PY" \
  || { echo "FAIL policy-scan-shell-quoted-eq-1: enforce missing allow_dq" >&2; exit 1; }
grep -qF 'allow_dq' "$RELEASE_PY" \
  || { echo "FAIL policy-scan-shell-quoted-eq-1: release-ready missing allow_dq" >&2; exit 1; }

tmp="$(mktemp -d "${TMPDIR:-/tmp}/pc-policy-scan-shell-quoted-eq-1-XXXXXX")"
trap 'rm -rf "$tmp"; rm -f "$ROOT/scripts/complete-e2e/.policy-scan-plant-certified-quoted-eq-1.sh"' EXIT

PLANT="$ROOT/scripts/complete-e2e/.policy-scan-plant-certified-quoted-eq-1.sh"
# Shell quoted ="1" enable shape (not CERTIFIED=1 / not CERTIFIED = 1 / not =true).
printf '%s\n' '#!/usr/bin/env bash' '# plant: policy-scan shell-quoted-eq-1 regression' 'CERTIFIED="1"' >"$PLANT"
...
assert "cert_dq" in src and "allow_dq" in src, "enforce must build quoted numeric needles"
assert '="1"' in src and "='1'" in src, "enforce must concat quoted numeric needles"
assert "ALLOW_UNPROVEN" in src and "certified" in src, "enforce must build ALLOW/CERTIFIED needles"


def scan(roots: list[str], suffixes: set[str], *, shell_quoted_eq_1: bool) -> list[str]:
    banned: list[str] = []
    for root_name in roots:
        ...
        for path in sorted(base.rglob("*")):
            ...
            elif shell_quoted_eq_1 and (allow_dq in text or allow_sq in text):
                banned.append(rel + ": " + allow_a)
            cert_a = "CERTIFIED" + "=1"
            cert_as = "certified" + " = 1"
            ...
            if cert_a in text or cert_as in text.lower() or cert_b in text.lower() or cert_t in text.lower():
                ...
                banned.append(rel + ": fake CERTIFIED enable")
            elif shell_quoted_eq_1 and (cert_dq in text.lower() or cert_sq in text.lower()):
                banned.append(rel + ": fake CERTIFIED enable")
    return banned


# Theater-kill: pre-quoted needles must miss the CERTIFIED="1" plant.
miss = scan(["scripts", "devtools", "configs", "media"], suffix_set, shell_quoted_eq_1=False)
assert not any(str(plant.relative_to(root)) in b for b in miss), (
    "sanity: pre-quoted needles unexpectedly caught CERTIFIED=\"1\" plant: " + repr(miss)
)

# Fixed needles must catch CERTIFIED="1" in the planted scripts/*.sh.
hit = scan(["scripts", "devtools", "configs", "media"], suffix_set, shell_quoted_eq_1=True)
plant_hits = [b for b in hit if str(plant.relative_to(root)) in b]
assert plant_hits, (
    "policy scan with shell CERTIFIED quoted =\"1\" needles must catch CERTIFIED=\"1\" "
...
for root_name in ("scripts", "devtools", "configs", "media"):
    ...
    for path in sorted(base.rglob("*")):
        ...
        if ("certified" + " = true") in text.lower() or ("certified" + "=true") in text.lower():
            banned.append(rel)
        if ("certified=" + '"1"') in text.lower() or ("certified=" + "'1'") in text.lower():
            banned.append(rel)
...
echo "PASS complete-e2e-policy-scan-covers-shell-quoted-eq-1: shell CERTIFIED/ALLOW_UNPROVEN=\"1\" enables fail-closed in enforce+release-ready policy scan (no scripts/*.sh quoted-=1-shape blind spot)"
