...
def main():
    ...
    live_regressions = [
        ...
        "tests/regression/complete-e2e-gitignore-active-bytecode.test.sh",
    ...
    # Layer 4 PASS while __pycache__/*.pyc/*.pyo lived only inside # comments.
    # Require __pycache__ AND *.pyc AND *.pyo: OR-base
    # (`__pycache__/` OR `*.pyc`) previously greenwashed Layer 4 PASS without
    ...
    def _gitignore_check_ignore_miss() -> list[str]:
        # Live git check-ignore: rule-text AND previously greenwashed Layer 4
    ...
    def _strip_bytecode_negations(raw: str) -> str:
        out = []
        for line in raw.splitlines(keepends=True):
            s = line.strip()
            if s.startswith("!") and any(
                tok in s for tok in ("__pycache__", ".pyc", ".pyo", "*.pyc", "*.pyo")
            ):
                continue
            out.append(line)
        return "".join(out)
    ...
    if has_pycache and has_pyc and has_pyo:
        miss = _gitignore_check_ignore_miss()
        if not miss:
            pass_(".gitignore covers bytecode")
        elif do_fix:
            cleaned = _strip_bytecode_negations(gi)
            if not cleaned.endswith("\n"):
                cleaned += "\n"
            # Re-ensure positive rules after stripping negations.
            rules2 = set()
            for raw in cleaned.splitlines():
                ...
                rules2.add(s)
            need = []
            if "__pycache__/" not in rules2 and "__pycache__" not in rules2:
                need.append("__pycache__/")
            if "*.pyc" not in rules2:
                need.append("*.pyc")
            if "*.pyo" not in rules2:
                need.append("*.pyo")
            if need:
                cleaned += "\n# Python bytecode\n" + "\n".join(need) + "\n"
            gi_path.write_text(cleaned, encoding="utf-8")
            info_("stripped bytecode negations / ensured rules in .gitignore")
            miss2 = _gitignore_check_ignore_miss()
            if miss2:
                fail_(".gitignore check-ignore still miss after --fix: " + ", ".join(miss2))
                rc = 1
            else:
                pass_(".gitignore covers bytecode")
        else:
    ...
    else: fail_(".gitignore missing active bytecode rules"); rc = 1
